...
| `_sla_headroom` | `1 - c/SLA`, c = the own prefill at the state's TTFT statistic (`E[S\|miss]` for the mean, `ttft_service_quantile` for a percentile); the latency ceiling's `k = 2(SLA - c)` vanishes at 0 and goes negative past it |
