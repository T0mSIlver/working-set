...
This is the single source of truth for the extended study (the `workingset`
package; `scripts/scenario_model.py` is a compatibility alias of this module). It keeps the SAME
...
MODELS = {
    ...
    # on it (see the note); nvfp4_w is measured on 3.8's own checkpoint.
    "27B": Model(
        ...
        # header: 23,417,339,744 B (dense MLPs of 56/64 layers NVFP4;
        # attention, DeltaNet, lm_head FP8; embed/MTP/vision BF16). Replaces
        # the projection of nvidia/Qwen3.6-27B-NVFP4's 21.92e9 B onto 3.8's
        # weights (no NVIDIA NVFP4 of 3.8 exists). Same whole-checkpoint
        # convention as the FP8 28.8 GiB, which the measured
        # Qwen/Qwen3.6-27B-FP8 checkpoint (30.87e9 B = 28.75 GiB) matches
        # within 0.2%. Dense: decode reads everything. research/nvfp4_2026-09.md.
    ...
    # PROJECTED from the encoder output, so prefill runs the encoder only
    # (8B active) while decode runs all 40 (16B). CSA2 shares FOUR compressed
    # caches across layers (sources 2/8/14 at ratio 2, 20 at ratio 1), main KV
    # in FP4 (E2M1 + E4M3/16 = 288 B/entry) and indexer keys in FP4 (68 B) ->
    # 890 B/token, the paper's own figure; plus 2.8 MiB/session of fp8
    # 128-entry windows on 43 layers and fp32 compressor state, carried in
    # deltanet_state. Decode reads four full-axis indexer scans (170 B/ctx
    # token) + top-512 latents on 38 layers, four candidate-pool (16,384
    ...
    "DSV41F": Model(
        ...
        w_decode_shared=8_522_921_408,   # exact ledger: attn 5.07 + shared exp 1.42 + gates/mHC/
                                         # compressors/indexers/engram-proj 0.71 + lm_head 1.32
        ...
        kv_decode_topk=1_024,            # 512 compressed entries x ratio 2, in token space
        kv_fp16_ok=False,                # FP4 main KV is the TRAINED cache format (QAT, tech
                                         # report 2.4.4); no stack serves a BF16 main KV
        max_ctx=1_048_576,               # native 1M (YaRN x16 over 65,536 baked into the config)
...
def _ttft_own(model, topo, wl, chunk, turn_tokens, mfu, per_pass_overhead,
    ...
    """E[S | miss] when `percentile` is None (a miss's mean TTFT), else the
    own-prefill term of the all-request percentile (ttft_service_quantile)
    — the one term the TTFT statistic changes."""
...
def prefill_ttft_seconds(model: Model, topo: Topology, wl, req_rate: float,
    ...
    `percentile` (None = a miss's mean TTFT): return a PROXY for the
    `percentile`-th TTFT over ALL requests instead — mean wait plus the
    own-prefill term of ttft_service_quantile. `request` must be left at
    "cold" then; the statistic covers both classes. See max_users_latency.
    ...
    if percentile is not None and request != "cold":
        raise ValueError("percentile is an all-request statistic; "
                         "leave request='cold'")
    own = (_ttft_own(model, topo, wl, chunk, turn_tokens, mfu,
...
def sla_miss_rate(model: Model, topo: Topology, wl, req_rate: float,
    ...
    (`percentile` set: whose all-request p-th TTFT proxy does, as in
    max_users_latency).
...
def max_users_latency(model: Model, topo: Topology, wl, chunk: float,
                      ...
                      decode_toks: float = DECODE_FLOOR_TOKS,
                      percentile: float | None = None) -> float:
    """Users at which the TTFT statistic reaches `sla_seconds`: a MISS's
    mean TTFT when `percentile` is None, else a proxy for the
    `percentile`-th TTFT over ALL requests.
    ...
    with a = E[S^2], b = E[S], c = E[S | miss] (or c_p), k = 2(SLA - c).
    ...
    a, b, c = (lambda m: (m[1], m[0], m[2]))(
        prefill_service_moments(model, topo, wl, chunk, turn_tokens, mfu,
                                per_pass_overhead))
    c_mean = c
    c = _ttft_own(model, topo, wl, chunk, turn_tokens, mfu, per_pass_overhead,
                  c_mean, percentile)
...
# ============================================================================
# SELF-CHECKS  (uv run python -m workingset.model, or `ws selfcheck`)
# ============================================================================
def _selfcheck():
    ...
    assert min_tp_for(MODELS["DSV41F"], "B300") == 2
    ...
    assert dsf.kv_decode_bpt == 3 * 68 / 2 + 68                 # four full-axis scans
    ...
    # same check for DSv4.1-Flash's compressed-sparse reads (8xH200: the
    # smallest H200 node split that holds its 510 GB)
