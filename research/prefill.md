...
| DeepSeek-V4-Flash-0731 *(added 2026-08-03; out 2026-09-10 → 09-16)* | 12.70e9 | 41 | 26,624 / 41 ≈ 649 | `model_dsv4flash.md` #6: active GEMM params excl. embed/lm_head (12.703e9 ledger). Quadratic term = indexer over the compressed axis (64 heads × 128 / 4 = 2,048, QK only, so 1024-equiv × 21 CSA layers) + dense HCA (64 × 512 / 128 = 256-equiv × 20); the CSA top-512 and window reads are linear and left out — priced *cheaper*, biased against the thrash hypothesis. |
| DeepSeek-V4.1-Flash *(added 2026-09-10)* | 7.90e9 | 3 | 32 × 128 / 2 / 2 = 1024 | `model_dsv41flash.md` #6: **encoder-only prefill** (Causal Encoder-Decoder — the decoder's global KV is projected from the encoder output, so prompt tokens never run layers 20–39): the 20 encoder layers' active GEMM params + the layer-20 projection, excl. embed/lm_head (exact shard ledger 7.896e9 ✓ the paper's "8B during prefill"; all 40 layers would be 15.9e9). Quadratic term = the three encoder Full-mode indexers (layers 2/8/14) scoring the whole ratio-2 compressed axis at 32 heads × 128 dims — QK only, so half the QK+AV width this table's convention assumes; the top-512 + window attention on all 18 encoder CSA2 layers is linear and left out, as is the fixed 128-token decoder replay per prefill — priced *cheaper*, biased against the thrash hypothesis. |
