...
// FP16 KV never on GLM-5.3 (vLLM's DSA path asserts a quantized cache).
...
function enforceConstraints(){
  ...
  renderSplitControl();
  // NVFP4 needs BOTH a capable GPU and an existing checkpoint (every model
  // has one as of 2026-09-06 — research/nvfp4_2026-09.md — the guard stays
  // for the next model that does not)
  ...
  document.getElementById('kv-tip').setAttribute('data-tip', fp16Ok
    ...
    : (state.model === "DSV4F"
      ? "DeepSeek-V4-Flash-0731: vLLM's V4 path asserts a quantized (FP8) main KV cache and SGLang's BF16 KV-decode is unfinished — FP16 KV is not servable, so the toggle is disabled."
      : state.model === "DSV41F"
  ...
  document.getElementById('state-tip').setAttribute('data-tip', stateOk
    ...
    : (state.model === "DSV4F"
      ? `DeepSeek-V4-Flash-0731 pays a fixed ${fmt(m.deltanet_state/MIB,1)} MiB/session (fp8/bf16 windows + fp32 compressor state), but its precision is set by the serving stack — no bf16/fp32 knob to turn, so the control is disabled.`
      : state.model === "DSV41F"
  ...
  const modelTips = {
    ...
    "GLM52": "GLM-5.3 — 744B-A40B MLA+DSA (constants derived on GLM-5.2, whose checkpoint is tensor-for-tensor the same shape): 47.3 KiB/token stored, but decode reads only top-2048 tokens/layer + an indexer scan. FP8 weights fit from 7×H200 / 4×B300 (recipe floor; 3×B300 by pool arithmetic alone); NVFP4 from 2×B300. 1M native context.",
    "DSV4F": "DeepSeek-V4-Flash-0731 — 284B-A13B, compressed sparse attention (21 CSA ratio-4 + 20 HCA ratio-128 + 2 SWA layers): only 3.4 KiB/token stored — ~10× below V3-class MLA — plus a fixed 14.9 MiB/session. The study's lightest big-MoE fit: from 2×H200 / 1×B300 (native FP4 experts; NVFP4 exists but is 5% heavier: the experts are already 4-bit natively and NVIDIA's repack only adds denser scales). A different architecture from V4.1-Flash (no encoder-decoder split, no Engram tables), kept alongside it. 1M native context.",
    "DSV41F": "DeepSeek-V4.1-Flash — 552B backbone + 196B Engram tables, 8B active in prefill / 16B in decode (Causal Encoder-Decoder: prefill runs the 20 encoder layers only). CSA2 shares four FP4 compressed caches across the 40 layers: 890 B/token stored — 4× below V4-Flash-0731, ~40× below V3-class MLA — plus a fixed 2.8 MiB/session. The study's KV-lightest model by far, but its heaviest fit after GLM-5.3: vLLM keeps the 203 GB Engram tables in HBM, so 510 GB resident — 8×H200 (5 by arithmetic) / 2×B300. Native MXFP4 experts (384, 6 routed: kink n = 64); no official NVFP4 checkpoint. 1M native context. Terminal-Bench: 90.6% on 2.1 and 31.2% on 4.0 are the vendor card's own runs (DeepSeek Harness Minimal, max effort) — the only non-Artificial-Analysis scores on the frontier, carried until AA publishes. On 2.1 expect AA to land lower, as it does for every other card; on 4.0 the direction is unknown — AA scores the two models it shares with this card ABOVE the card (12.1 vs 7.0, 41.9 vs 37.9).",
    "Q38FN": "Qwen3.8-Flash-Next — MoE 125B-A6B (+51B FP8 n-gram table): 48 layers (36 DeltaNet + 12 QSA sparse attention) → 12.4 KiB/token stored, decode reads top-2048 tokens + an indexer scan; 512 experts / 10 routed (deepest expert-union kink, n = 51.2) and a 56.8 MiB bf16 state per session. Fits from 2×H200; 1×B300 by pool arithmetic (recipe floor TP2 — TP1 hit compilation OOM on GB300). NVFP4: NVIDIA checkpoint (2026-09-02), routed experts only, ×0.5625 per expert — 132.6 vs 185.5 GB resident, fixed read unchanged. 262k native context, 1M via YaRN.",
    "GLM53F": "GLM-5.3-Flash — MoE 320B-A18B hybrid (34 KDA linear + 11 NoPE sparse-MLA layers, +1 MTP DSA layer whose cache is charged): 6.4 KiB/token stored — 7.4× below GLM-5.2 — plus a 74.4 MiB bf16 KDA state per session; decode reads top-2048 latents + a kpool-4 compressed indexer scan. 288 experts / 8 routed (kink n = 36). Fits from 3×H200 / 2×B300 (pool arithmetic; no recipe floor published). fp8 KV is Blackwell-only, so on H200 the KV toggle locks to BF16. NVFP4: RedHatAI checkpoint (2026-08-27, the vLLM recipe\'s) — 42 decode layers\' experts ×0.5625, the rest upcast to BF16, so the fixed per-step read grows 14.0 → 16.7 GB while the resident drops to 197.8 GB. 1M native context.",
  };
  ...
  // 1M-context models (Qwens via YaRN, GLM-5.3) allow 4× the Mistral ranges.
...
// change that moves one axis). #seg-bench carries no data-key, so the generic
// .seg handler above leaves it alone.
document.querySelectorAll('#seg-bench button').forEach(btn=>{
  btn.addEventListener('click',()=>{
    if (state.bench === btn.dataset.v) return;
    state.bench = btn.dataset.v;
    document.querySelectorAll('#seg-bench button').forEach(
...
function syncLabels(){
  ...
  // GLM-5.3 drafts 5 tokens, DSv4-Flash 7 and DSv4.1-Flash 5 (DSpark), Q3.8-Flash-Next 3, and Mistral
  // has no MTP module (its slider models EAGLE) — indicative on all of those.
  const aNote = state.model==="GLM52" ? ' GLM-5.3 drafts 5 tokens — α is indicative only.'
              : state.model==="DSV4F" ? ' DSv4-Flash drafts 7 tokens (DSpark) — α is indicative only.'
              : state.model==="DSV41F" ? ' DSv4.1-Flash drafts 5 tokens (DSpark) — α is indicative only.'
  ...
  function handler(e){
    ...
    tt.innerHTML=`<div class="tth">${esc(frontierRowName(r))}${r.key===geom.curKey?' — yours':''}</div>`
      ...
      +Object.entries(CONFIG.BENCHES)
        .sort(([a],[b]) => (a===state.bench?0:1)-(b===state.bench?0:1))
        .map(([k,b]) => {
          const q = frontierScore(r, k), on = k===state.bench;
          return `<div class="row"${on?'':' style="opacity:.65"'}>${esc(b.name)}${on?' (axis)':''} `
               + `<b class="tnum">${isFinite(q) ? fmt(q*100,1)+'%' : '—'}</b></div>`;
        }).join('')
