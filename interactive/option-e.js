// Option E — experiment notebook. Reads the page the main modules render and
// re-presents three pieces of it; it never computes a number of its own.
...
import { state, STATE_DEFAULTS } from './src/state.js';
...
function readout(){
  ...
  $('calDefaults').textContent =
    `model defaults: prefill ${Number(d.mfu).toFixed(2)} · decode ${Number(d.mbu).toFixed(2)}`
    + `  —  now: ${Number(state.mfu).toFixed(2)} · ${Number(state.mbu).toFixed(2)}`;
}
